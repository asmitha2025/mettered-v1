...
def run_step(name, cmd, records=None):
    ...
    if result.returncode != 0:
        print(f"\n  [ERROR] {name} FAILED (exit code {result.returncode})", flush=True)
        sys.exit(result.returncode)

    print(f"\n  [TIME] {name} completed in {elapsed}s", flush=True)
...
def main():
    ...
    try:
        ...
        if SparkSession and java_available() and spark_runtime_allowed(log_reason=False):
...
|   --> Auto-selected when Spark is unavailable or local    |
|       Spark writes are not safely configured.             |"""
