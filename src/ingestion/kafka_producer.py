...
mirroring how a billing platform processes webhook events in real time.
...
def produce_dry_run(events_per_second=5, duration_seconds=10):
    """Print events to stdout without requiring Kafka."""
    print(f"\n[DRY RUN] Producing {events_per_second} events/sec for {duration_seconds}s\n")
    total = 0
    for _ in range(duration_seconds):
        for _ in range(events_per_second):
            ...
            print(
                ...
                f"tenant={event['tenant_id']} amount=INR {event['amount']}"
            )
            total += 1
        time.sleep(1)
    print(f"\n  [OK] Produced {total} events (dry-run)\n")


def produce_to_kafka(topic: str = "billing-events", events_per_second: int = 10):
    """Push events to Kafka. Requires kafka-python."""
    try:
        from kafka import KafkaProducer
    except ImportError:
        print("  [WARN] kafka-python not installed. Run: pip install kafka-python")
        print("  [TIP] Use --dry-run to simulate without Kafka.")
        return
    ...
    print(f"\n[STREAM] Kafka topic: '{topic}' at {events_per_second} events/sec")
    ...
    try:
        ...
        print(f"\n\n  [OK] Stopped. Total events produced: {produced}\n")
    finally:
        producer.close()
